...
import { ArrowLeftIcon, CategoryIcon, ClockIcon, PencilIcon, PlusIcon, TrashIcon } from '../components/Icons'
...
export default function DocViewer() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            supabase.from('attachments').select('*').eq('doc_id', id).order('created_at', { ascending: false })
        ]).then(([docRes, attRes]) => {
    ...
    const handleSave = async (title: string, content: string) => {
        if (doc) {
            await supabase.from('torq_docs').update({
                title, content_markdown: content, updated_by: user?.id
            }).eq('id', doc.id)
    ...
    const handleRestore = async (title: string, content: string) => {
        if (!doc || !user) return
        const { error } = await supabase.from('torq_docs')
            .update({ title, content_markdown: content, updated_by: user.id }).eq('id', doc.id)
        if (!error) {
    ...
    if (editing) return (
        <DocEditor
            ...
            onSave={handleSave}
            onCancel={() => { setEditing(false); navigate(`/doc/${id}`, { replace: true }) }}
        />
    )
            ...
            <div className="doc-viewer">
                <header className="doc-header">
                    <div className="doc-breadcrumb">
                        <button className="btn-ghost breadcrumb-back btn-with-icon" onClick={() => navigate(`/?cat=${doc.categories?.slug}`)}>
                    ...
                    <div className="doc-header-row">
                        ...
                        <div className="doc-actions">
                            <button className="btn-ghost doc-action-btn btn-with-icon" onClick={() => setShowHistory(true)}>
                                <ClockIcon size={14} />
                                Histórico
                            </button>
                            {isAdmin && (
                                <>
                                    <button className="btn-ghost doc-action-btn btn-with-icon" onClick={async () => {
                                        if (!doc || !user) return
                                        const { data } = await supabase.from('torq_docs')
                                            .insert({ title: 'Novo Documento', content_markdown: '# Novo Documento\n\n', category_id: doc.category_id, created_by: user.id, updated_by: user.id })
                                            .select().single()
                                    ...
                                    <button className="btn-primary doc-action-btn btn-with-icon" onClick={() => setEditing(true)}>
                                        <PencilIcon size={14} />
                                        Editar
                                    </button>
                                    <button className="btn-ghost doc-action-btn btn-with-icon btn-danger" onClick={async () => {
                                        if (!doc) return
                                        const confirmed = window.confirm(`Tem certeza que deseja excluir "${doc.title}"? Esta ação não pode ser desfeita.`)
                                        if (!confirmed) return
                                        try {
                    ...
                    <div className="doc-meta">
                        ...
                        <span className="doc-date">
                            {new Date(doc.updated_at).toLocaleDateString('pt-BR', { year: 'numeric', month: 'long', day: 'numeric' })}
                        </span>
                    </div>
                </header>
