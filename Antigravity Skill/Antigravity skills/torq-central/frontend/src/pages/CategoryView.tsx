...
export default function CategoryView() {
  ...
  const handleFolderDrop = useCallback(
    async (e: React.DragEvent, targetFolder: Category) => {
      e.preventDefault()
      setDropTargetId(null)
    ...
    <div className="category-view">
      <header className="category-view-header">
        ...
        <p className="category-view-meta">
          {subFolders.length > 0 &&
            `${subFolders.length} pasta${subFolders.length > 1 ? 's' : ''} · `}
      ...
      {movingMessage && <div className="move-toast">{movingMessage}</div>}
      ...
      {subFolders.length > 0 && (
        <section className="cards-section">
          <div className="cards-section-title">
            Pastas
            {draggingDocId && (
              <span className="drop-hint">Solte o documento sobre uma pasta para mover</span>
            )}
          </div>
        ...
        <div className="cards-grid">
          {docs.length === 0 && (
            <div className="cards-empty">
              ...
              {isAdmin
                ? 'Nenhum documento. Clique em "Novo Documento" para começar.'
          ...
          {docs.map((doc) => (
            ...
            >
              {isAdmin && (
                <span className="drag-handle" title="Arraste para mover para outra pasta">
                  ⠿
                </span>
              )}
