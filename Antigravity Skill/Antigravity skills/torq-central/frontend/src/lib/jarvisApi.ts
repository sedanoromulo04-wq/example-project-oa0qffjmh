...
export async function listJarvisSessions(
  ...
  const data = await request<{ ok: true; sessions: JarvisSession[] }>(
    `/api/jarvis/sessions?user_id=${encodeURIComponent(userId)}${emailPart}`,
  )
  return data.sessions
}

export async function getJarvisSession(
  userId: string,
  sessionId: string,
  userEmail?: string | null,
) {
...
export async function reviewJarvisSessionActions(payload: {
  ...
  return request<
    {
      ok: true
    } & JarvisActionReviewResult
  >(`/api/jarvis/sessions/${encodeURIComponent(payload.session_id)}/actions/review`, {
    method: 'POST',
    body: JSON.stringify(payload),
  })
}
