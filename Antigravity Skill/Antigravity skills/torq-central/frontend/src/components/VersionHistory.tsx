...
export default function VersionHistory({ docId, onRestore, onClose, isAdmin }: Props) {
    ...
    const handleRestore = (v: Version) => {
        if (confirm(`Restaurar para a versão #${v.version_number}? O conteúdo atual será salvo como nova versão.`)) {
                        ...
                        >
                            ...
                            {preview?.id === v.id && (
                                <div className="history-preview">
                                    <pre className="history-preview-content">{v.content_markdown.slice(0, 400)}{v.content_markdown.length > 400 ? '...' : ''}</pre>
                                    {isAdmin && (
                                        <button
